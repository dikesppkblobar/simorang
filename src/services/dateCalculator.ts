...
export function getStrSipExpiryStatus(
  noStr?: string,
  tglAkhirStr?: string,
  isSeumurHidup?: boolean,
  noSip?: string,
  tglAkhirSip?: string
) {
  const now = new Date();

  let strStatus: { text: string; badgeColor: string; isWarning: boolean; isExpired: boolean } = {
    text: 'Belum Ada STR',
  ...
  if (isSeumurHidup) {
    strStatus = {
      text: 'STR Seumur Hidup (Aktif)',
      badgeColor: 'bg-emerald-50 text-emerald-700 border-emerald-200',
      isWarning: false,
      isExpired: false,
    };
  } else if (noStr && tglAkhirStr) {
    const expDate = parseDate(tglAkhirStr);
    const daysLeft = getDaysBetween(now, expDate);
    if (daysLeft < 0) {
      strStatus = {
        text: `STR Kadaluarsa (${Math.abs(daysLeft)} hari lalu)`,
      ...
      strStatus = {
        text: `STR Habis Dalam ${daysLeft} Hari`,
      ...
      strStatus = {
        text: `STR Aktif s.d ${formatDateIndonesian(tglAkhirStr)}`,
        badgeColor: 'bg-emerald-50 text-emerald-700 border-emerald-200',
        isWarning: false,
        isExpired: false,
      };
  ...
  let sipStatus: { text: string; badgeColor: string; isWarning: boolean; isExpired: boolean } = {
    text: 'Belum Ada SIP',
    badgeColor: 'bg-slate-100 text-slate-600 border-slate-200',
    isWarning: false,
    isExpired: false,
  ...
  if (noSip && tglAkhirSip) {
    const expDate = parseDate(tglAkhirSip);
    const daysLeft = getDaysBetween(now, expDate);
    if (daysLeft < 0) {
      sipStatus = {
        text: `SIP Kadaluarsa (${Math.abs(daysLeft)} hari lalu)`,
        badgeColor: 'bg-rose-50 text-rose-700 border-rose-200',
        isWarning: true,
        isExpired: true,
      };
      ...
      sipStatus = {
        text: `SIP Habis Dalam ${daysLeft} Hari`,
        badgeColor: 'bg-amber-50 text-amber-700 border-amber-200',
        isWarning: true,
        isExpired: false,
      };
      ...
      sipStatus = {
        text: `SIP Aktif s.d ${formatDateIndonesian(tglAkhirSip)}`,
